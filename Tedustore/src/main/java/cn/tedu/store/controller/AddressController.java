...
public class AddressController extends BaseController {
	...
	// 跨域!!!!!!!!!!!***************
	@RequestMapping(value = "/addressmsg2.do", produces = { "text/html;charset=UTF-8;" })
	@ResponseBody
	public String addressmsg2(HttpSession session, String str, HttpServletRequest request,
		...
		try {
			rr.setState(1);
			rr.setMessage("保存成功");
			List<Address> addresslist = addressService.selectAddressByUid(1, "1");
			rr.setData(addresslist);
			result = addressService.getJsonPData(callback, rr);
		} catch (Exception e) {
			rr.setState(0);
			rr.setMessage(e.getMessage());
			e.printStackTrace();
