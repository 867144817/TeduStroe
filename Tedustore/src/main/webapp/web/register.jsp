...
<body>
	...
	<header id="top">
		<div class="top">
			<img src="../images/header/logo.png" alt="" /> <span>欢迎注册</span>
		</div>
	</header>
	<div class="parent">
		...
		<div class="container">
			<div class="panel rt">
				<form id="form-register" method="post"
					...
					<div class="txt">
						<p>
							新用户注册 <span> <a href="../user/showLogin.do">直接登录</a>
							</span>
						</p>
					</div>
					<div class="form-group">
						<label for="uname">用户名：</label> <input autocomplete required
							minlength="2" maxlength="9" type="text" placeholder="请输入用户名"
							autofocus name="username" id="uname" /> <span class="msg-default">用户名长度在6到9位之间</span>
					</div>
					<div class="form-group">
			...
			$.ajax({
				...
				success : function(obj) {
					...
					if (obj.state == 1) {
						...
						if(obj.message=="该用户名已被占用"){
							doResponse(obj, $("#uname"));
						}else if(obj.message=="邮箱已存在"){
							doResponse(obj, $("#email"));
						}else if(obj.message=="电话已存在"){
							doResponse(obj, $("#phone"));
						}else{
							alert(obj.message);
						}
					}
	...
	<script>
		...
		uname.onblur = function() {
			if (this.validity.valueMissing) {
				...
				$.ajax({
					url : "../user/checkUsername.do",//获取地址
					type : "post",//请求方式
					data : "username=" + this.value,//请求数据
					dataType : "json",//请求格式
					success : function(obj) {
						console.log('响应完成且成功');
						doResponse(obj, $("#uname"));
		...
		//处理响应消息
		function doResponse(obj, t) {
			console.log('开始处理响应数据');
			//console.log(xhr);
			t.next().html(obj.message);
			if (obj.state == 0) {
				t.next().attr("class", "msg-error");
				//.addClass("msg-error");
			} else if (obj.state == 1) {
				t.next().attr("class", "msg-success");
			} else {
				alert(obj);
			}
		}
