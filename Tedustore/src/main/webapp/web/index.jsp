...
<body>
	...
	<div class="lf1">
		...
		<div class="lf1_bottom">
			...
			<div class="item_cat lf">
				...
				<div class="item_cat_all">
					<c:forEach items="${list2 }" var="list2" varStatus="s">
						<p>${list2.name }</p>
						<ul>
							<c:forEach items="${list3[s.index] }" var="list3">
										<li><a href="../goods/showSearch.do?categoryId=${list3.id }">${list3.name }</a></li>
							</c:forEach>
						</ul>
					</c:forEach>
				</div>
			...
			<c:forEach items="${listx }" var="lx">
				<div class="item_msg lf">
					<img src="..${lx.image }" alt="" class="w198"/>
					<p class="bottom_ys2">${lx.title }</p>
					<p class="bottom_ys3">￥${lx.price }.00</p>
